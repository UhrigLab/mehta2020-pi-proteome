########################################################################################################################
#                                                                                                                      #
# pepseqfinder is a function that uses a list of peptides that were modified to look up aminoacid sequences of fixed #
# length in a translatome. It converts the list into a list of unmodified peptides, creates a blast database, uses     #
# blastp to find the unmodified peptides, and then searches for each modification and creates the aminoacid sequences  #
# around the modification. The script has only two parts the __main__ which is the entry point of the script, handling #
# In and out, and the script pepseqfinder itself.                                                                    #
#                                                                                                                      #
########################################################################################################################
#
#
...
#   File at the path specified with voutput_file_name containing all the results in [tab] separated format. Organization
...
#
# Call:
#   python pepseqfinder.py [vmod_peptides_file] [vmod_spec_file] [vtranslatome_file] [vrange] [voutput_file_name]
#
# Author information:
# June 2019
...
def pepseqfinder(vmod_peptides_file, vmod_spec_file, vtranslatome_file, vrange, voutput_file_name):

    vverbose = 3 # standard setting for screen output is minimal (more screen output the higher the number)
    ...
    except IOError:  # If file does not exist or can not be read (permission error)
    ...
    for row in vmodlist:  # for each modification, starting with the top line in the file
        ...
        print(re.escape(row[0]))
        print(re.escape(row[1]))
    ...
    try:
        ...
        print('Cannot write output file ' + str(voutput_file_name))
        sys.exit()
    ...
    for row in vbr2:
        ...
        while vnothingfound == 0:  # indicate that as long as something is found, continue to search for something else.
            ...
            if minvid == 1000000000:  # if the minimal position is the arbitrary number, there is no modification left
                ...
                # extended sequence
                print('Processing result ' + str(vrid) + ' of ' + str(len(vbr2)))
...
if __name__ == "__main__":
    ...
    v4 = sys.argv[4]
    v5 = sys.argv[5]
    pepseqfinder(v1, v2, v3, v4, v5)  # Call of the function
